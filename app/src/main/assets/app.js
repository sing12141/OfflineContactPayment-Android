  ...
  try{
    ...
    contacts.forEach(c=>{
      ...
      if(c.calledAt&&/\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4}/.test(c.calledAt)){
        ...
        c.calledAt=String(m[1]).padStart(2,'0')+'/'+
                   String(m[2]).padStart(2,'0')+'/'+m[3];
      }
    });
...
function unpaid(c){
  return years.filter(y=>String(c.data[y]??'').trim()==='');
}

function renderFilters(){
  const s=$('yearFilter');
  s.innerHTML='<option value="all">All contacts</option>'+
...
function saveRemark(i){
  const el=document.querySelector('[data-remark="'+i+'"]');
  if(!el)return;
  ...
  c.remarksHistory=Array.isArray(c.remarksHistory)
    ?c.remarksHistory
    :((c.remarks||'')?[c.remarks]:[]);
...
function stats(){
  ...
  const full=contacts.filter(c=>unpaid(c).length===0).length;
  ...
  $('stats').innerHTML=
    `<button class="stat statBtn ${activeView==='total'?'active':''}"
...
function render(){
  ...
  let arr=contacts.filter(c=>
    (
      activeView==='total'||
      (activeView==='notCalled'&&!c.called)||
      (activeView==='called'&&c.called)
    )
    &&
    (
      !q||
      ...
      String(c.remarks||'').toLowerCase().includes(q)||
      ((c.remarksHistory||[]).join(' ')).toLowerCase().includes(q)
    )
    &&
    (
      yf==='all'||unpaid(c).includes(yf)
    )
  );
  ...
  if(!arr.length){
    $('list').innerHTML=
      '<div class="empty">No contacts found.<br>Import an Excel file to begin.</div>';
    return;
  }
  ...
  $('list').innerHTML=arr.map(c=>{
    ...
    return `
    <article class="card ${c.called?'called':''}"
      data-card="${i}">
      ...
      <div class="years">
        ...
        ${years.map(y=>{
          ...
          return `
          <span class="year ${ok?'yearPaid':'yearUnpaid'}">
            <b>${esc(y)}</b>:
            ${ok?'PAID':'NOT PAID'}
          </span>`;
      ...
      <div class="yearSummary">
        ...
        <span class="paidText">
          <b>Paid years:</b>
          ${paidYears.length?paidYears.join(', '):'None'}
        </span>

        <span class="unpaidText">
          <b>Unpaid years:</b>
          ${u.length?u.join(', '):'None'}
        </span>
...
function callNumber(p){
  location.href='tel:'+String(p).replace(/[^\d+]/g,'');
}
...
async function readZip(buf){
  ...
  if(eocd<0)
    throw Error('Invalid XLSX/ZIP file');
  ...
  while(p<cdOff+cdSize){
    ...
    const us=dv.getUint32(p+24,true);
  ...
  async function get(name){
    ...
    const n=dv.getUint16(q+26,true);
    const m=dv.getUint16(q+28,true);

    const start=q+30+n+m;

    const raw=
      b.slice(start,start+f.cs);
...
async function parseXlsx(buf){
  ...
  if(!z.files[sheet]){
    ...
    if(!names.length)
      throw Error('No worksheet found');
...
async function importFile(file){
  ...
  if(!file)
    throw Error('Please select an Excel file.');
...
function zipStored(entries){
  ...
  const l=joinBytes(lo);
  const c=joinBytes(ce);
...
function makeSheet(){
  ...
  rows.forEach((r,ri)=>{
    ...
    r.forEach((v,ci)=>{
      ...
      x+=
        `<c r="${colName(ci+1)}${ri+1}" t="inlineStr">`+
        `<is><t>${esc(v)}</t></is></c>`;
    });
